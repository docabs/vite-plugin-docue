...
// import jsx from '@docue/babel-plugin-jsx'
...
const ssrRegisterHelperId = '/__docue-jsx-ssr-register-helper'
const ssrRegisterHelperCode =
  `import { useSSRContext } from "docuejs"\n` +
...
function docueJsxPlugin(options: Options = {}): Plugin {
  let root = ''
  let needHmr = false
  ...
  return {
    ...
    async transform(code, id, opt) {
      ...
      // use id for script blocks in Docue SFCs (e.g. `App.docue?docue&type=script&lang.jsx`)
      ...
      if (filter(id) || filter(filepath)) {
        ...
        if (hotComponents.length) {
          ...
          if (needHmr && !ssr && !/\?docue&type=script/.test(id)) {
            let code = result.code
            let callbackCode = ``
            for (const { local, exported, id } of hotComponents) {
              code +=
                `\n${local}.__hmrId = "${id}"` +
                `\n__DOCUE_HMR_RUNTIME__.createRecord("${id}", ${local})`
              callbackCode += `\n__DOCUE_HMR_RUNTIME__.reload("${id}", __${exported})`
            }
