...
export async function transformMain(
  ...
  if (fs.existsSync(filename)) {
    ...
    getDescriptor(
      ...
      true,
      true,
      // for docue files, create descriptor from fs read to be consistent with
      // logic in handleHotUpdate()
      // for non docue files, e.g. md files in vitepress, we assume
      ...
      // code here as well.
      filename.endsWith('.docue') ? undefined : code,
    )
  }
...
async function genTemplateCode(
  ...
  if ((!template.lang || template.lang === 'html') && !template.src) {
    ...
    const query = `?docue&type=template${srcQuery}${scopedQuery}${attrsQuery}`
