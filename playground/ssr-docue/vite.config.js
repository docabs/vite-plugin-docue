...
import docueJsx from 'vite-plugin-docue-jsx'
...
export default defineConfig(({ command, ssrBuild, isSsrBuild }) => ({
  ...
  plugins: [
    ...
    (function () {
      ...
      const virtualId = '\0virtual:ssr-docue-built-url'
      return {
        name: 'built-url',
        enforce: 'post',
        ...
        load(id) {
          if (id === virtualId) {
            return {
              code: `export const __ssr_docue_processAssetPath = (url) => '${base}' + url`,
        ...
        transform(code, id) {
          ...
          if (
            config.build.ssr &&
            (cleanId.endsWith('.js') || cleanId.endsWith('.docue')) &&
            !code.includes('__ssr_docue_processAssetPath')
          ) {
            return {
              code:
                `import { __ssr_docue_processAssetPath } from '${virtualId}';__ssr_docue_processAssetPath;` +
                code,
  ...
  experimental: {
    renderBuiltUrl(filename, { hostType, type, ssr }) {
      if (ssr && type === 'asset' && hostType === 'js') {
        return {
          runtime: `__ssr_docue_processAssetPath(${JSON.stringify(filename)})`,
        }
      }
    },
  },
  ...
  ssr: {
    noExternal: [
      // this package has uncompiled .docue files
